...
class AfnSettingsPanel(sublime_plugin.WindowCommand):
    ...
    def run(self):
        use_pr = '✗ Stop using project root' if self.get_setting(
            'afn_use_project_root') else '✓ Use Project Root'
        use_dim = '✗ Disable HTML Image Dimension insertion' if self.get_setting(
            'afn_insert_dimensions') else '✓ Auto-insert Image Dimensions in HTML'
...
class InsertDimensionsCommand(sublime_plugin.TextCommand):
    ...
    # determines if there is a template tag in a given region.  supports HTML
    ...
    def run(self, edit):
        ...
        tag_scope = view.line(sel) if self.get_setting(
            'afn_template_languages', view) else view.extract_scope(scope.a - 1)
...
class FileNameComplete(sublime_plugin.EventListener):
    ...
    def get_drives(self):
        # Search through valid drive names and see if they exist. (stolen from
        # Facelessuser)
    ...
    def on_query_context(self, view, key, operator, operand, match_all):
        ...
        if key == "afn_deleting_slash":  # for reloading autocomplete
            sel = view.sel()[0]
            valid = self.at_path_end(view) and sel.empty() and view.substr(
                sel.a - 1) == FileNameComplete.sep
    ...
    def get_cur_path(self, view, sel):
        ...
        return cur_path[:cur_path.rfind(FileNameComplete.sep) + 1] if FileNameComplete.sep in cur_path else cur_path
